...
SCALAR_DENSITY_CORRECTION = (
    2.15  # adjust normalization of density correction to match 10mK noise simulation
)


def get_args(argv=None, include_plot=True):
    ...
    file_group.add_argument(
        "--basedir",
        default="/astro/mwaeor/MWA/output/",
    ...
    plot_group.add_argument(
        ...
        "2D_diff, 2D_Ratio_Diff, 1D_comp. Defaults to 2D",
        default="2D",
    )
    plot_group.add_argument(
        "--polarisation",
    ...
    plot_group_1D.add_argument(
        ...
        default=100,
        type=float,
        help="Maximum k_parra to use in 1D averaging. Can use this to "
    ...
    chips_group.add_argument(
        "--N_kperp",
        type=int,
        default=80,
        help="The number of kperp bins used in CHIPS fft command. Default=80",
    )
    chips_group.add_argument(
        "--N_chan_orig",
        type=int,
        default=None,
        help=(
            "The number of input channels given to CHIPS fft. "
            "default: use --chan_width to guess."
        ),
    )
    chips_group.add_argument(
        "--N_chan",
        ...
        default=None,
        help=(
            "The number of output channels from CHIPS fft. "
            "default: use --N_chan_orig and --bias_mode to guess."
        ),
    )
    ...
    chips_group.add_argument(
        ...
        default=167.035e6,
        type=float,
        help="Lowest input channel given to CHIPS fft (Hz). Default is eor highband 167.035e+6",
    )
    chips_group.add_argument(
        "--lowerfreq",
        default=None,
        type=float,
        help=(
            "Lowest output channel from CHIPS fft (Hz). "
            "Default: guess from --lowerfreq_orig, --N_chan_orig, --chan_width and --bias_mode."
        ),
    )
    chips_group.add_argument(
        "--chan_width",
    ...
    chips_group.add_argument(
        ...
        default=300.0,
        type=float,
        help="Maximum u-value used in CHIPS fft stage (wavelengths). Default = 300",
    )
    chips_group.add_argument(
        "--density_correction",
        default=SCALAR_DENSITY_CORRECTION,
    ...
    chips_group.add_argument(
        ...
        default=None,
        type=int,
        help="Starting channel index for fft if using lssa_fg_general. Default = None",
    )
    chips_group.add_argument(
        "--num_obs",
        default=1,
    ...
    chips_group.add_argument(
        ...
        default=None,
        type=float,
        help="Dark energy dentisty parameter. Default = 1 - omega_matter - omega_baryon",
    )
    chips_group.add_argument(
        "--hubble",
        default=70.4,
    ...
    if args.omega_lambda is None:
        # Ω_Λ + Ω_b + Ω_m = 1
        args.omega_lambda = 1 - args.omega_matter - args.omega_baryon
    args.verbose = True
    ...
    # guess nchan_orig if not given
    if args.N_chan_orig is None:
        mwa_bandwidth = 30.72e6
        args.N_chan_orig = round(mwa_bandwidth / args.chan_width)
        assert args.N_chan_orig >= 8, "N_chan_orig should probably be at least 8"
        assert args.N_chan_orig % 8 == 0, "N_chan_orig should probably be even"

    if args.bias_mode == 0:
        args.N_chan = args.N_chan_orig
        args.start_chan = 0
    ...
    elif args.bias_mode == 13:
        args.N_chan = round(args.N_chan_orig / 2)
        args.start_chan = round(args.N_chan_orig / 8)
    elif args.N_chan is None or args.start_chan is None:
        raise ValueError(
